...
int asCScriptEngine::SetMessageCallback(const asSFuncPtr &callback, void *obj, asDWORD callConv)
{
	...
	if( (unsigned)callConv >= asCALL_THISCALL && (unsigned)callConv < asCALL_DDECL )
	{
		isObj = true;
		if( obj == 0 )
		{
...
void *asCScriptEngine::CallObjectMethodRetPtr(void *obj, int param1, asCScriptFunction *func) const
{
	...
	else if( i->callConv == ICC_CDECL_OBJLAST || i->callConv == ICC_DDECL_OBJLAST )
	{
...
{
	if( i->callConv == ICC_CDECL || i->callConv == ICC_DDECL )
	{
		void *(*f)() = (void *(*)())(i->func);
		return f();
	}
...
{
#if defined(__GNUC__) || defined(AS_PSVITA)
	if( i->callConv == ICC_CDECL_OBJLAST || i->callConv == ICC_DDECL_OBJLAST )
	{
