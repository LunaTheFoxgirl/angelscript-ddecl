...
int DetectCallingConvention(bool isMethod, const asSFuncPtr &ptr, int callConv, void *auxiliary, asSSystemFunctionInterface *internal)
{
	...
	if( !isMethod )
	{
		...
		else if( base == asCALL_DDECL )
			internal->callConv = ICC_DDECL;
	...
	if( isMethod )
	{
		...
		else if( base == asCALL_DDECL_OBJLAST )
			internal->callConv = ICC_DDECL_OBJLAST;
		else if( base == asCALL_DDECL_OBJFIRST )
			internal->callConv = ICC_DDECL_OBJFIRST;
		else if (base == asCALL_GENERIC)
		{
	...
	else if( func->returnType.IsObject() )
	{
		...
		else if( objType & asOBJ_APP_CLASS )
		{
			...
			else
			{
				...
				if((internal->callConv == ICC_DDECL         ||
					internal->callConv == ICC_DDECL_OBJLAST ||
					internal->callConv == ICC_DDECL_OBJFIRST) &&
					func->returnType.GetSizeInMemoryDWords() >= DDECL_RETURN_SIMPLE_IN_MEMORY_MIN_SIZE)
				{
					internal->hostReturnInMemory = true;
					internal->hostReturnSize     = sizeof(void*)/4;
				}
